...
class System():
    ...
    def __init__(self, file="", dirname="", rootname="", depth=0, rho=1025, g=9.81, qs=1, Fortran=True, MDversion=1):
    ...
    def load(self, filename, MDversion):
            ...
            for line in f:          # loop through each line in the file
                ...
                if line.count('---') > 0 and (line.upper().count('BODIES') > 0 or line.upper().count('BODY LIST') > 0 or line.upper().count('BODY PROPERTIES') > 0):
                    ...
                    while line.count('---') == 0:
                        ...
                        num = int("".join(c for c in entry0 if not c.isalpha()))  # remove alpha characters to identify Body #
                ...
                if line.count('---') > 0 and (line.upper().count('RODS') > 0 or line.upper().count('ROD LIST') > 0 or line.upper().count('ROD PROPERTIES') > 0):
                    ...
                    while line.count('---') == 0:
                        ...
                        num = int("".join(c for c in entry0 if not c.isalpha()))  # remove alpha characters to identify Rod #
                ...
                if line.count('---') > 0 and (line.upper().count('POINTS') > 0 or line.upper().count('POINT LIST') > 0 or line.upper().count('POINT PROPERTIES') > 0 or line.upper().count('CONNECTION PROPERTIES') > 0):
                    line = next(f) # skip this header line, plus channel names and units lines
                    line = next(f)
                    line = next(f)
                ...
                if line.count('---') > 0 and (line.upper().count('LINES') > 0 or line.upper().count('LINE LIST') > 0 or line.upper().count('LINE PROPERTIES') > 0):
                    ...
                    while line.count('---') == 0:
                        ...
                        self.lineList.append( Line(self, num, lUnstr, self.lineTypes[entries[1]].name, nSegs=nSegs, attachments = [int(entries[4]),int(entries[5])]) )
                        
                        # attach ends
                        self.pointList[int(entries[4])-1].attachLine(num, 0)
                        self.pointList[int(entries[5])-1].attachLine(num, 1)
